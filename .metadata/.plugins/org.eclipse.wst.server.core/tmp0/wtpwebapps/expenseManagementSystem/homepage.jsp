		...
		<table class="table">
		  ...
		  <tbody>
		    ...
		    <tr>
		      ...
		      <td><%=e.getDesc() %></td>
		      <td><%=e.getAmount() %></td>
		      <td><a href="" class="link1" data-toggle="modal" data-target="#delete_list" onclick="addToModal(<%=e.getId() %>,'<%=expenseType%>')">Delete</a></td>
		    </tr>
		    <% i++;} %>
		  </tbody>
		</table>
...
<% if (eList.size() !=0){%>
	<div class="m-4">
		<div class="progress" style="height: 35px; font-weight: bolder">
		  <div class="progress-bar" role="progressbar" style="width: <%= ((travel/size)*100.0) %>%">Travel (<%= (int)((travel/size)*100.0)  %>) %</div>
		  <div class="progress-bar bg-success" role="progressbar" style="width: <%= ((food/size)*100.0) %>%">Food (<%= (int)((food/size)*100.0) %>) %</div>
		  <div class="progress-bar bg-danger" role="progressbar" style="width: <%= ((medicine/size)*100.0) %>%">Medicines (<%= (int)((medicine/size)*100.0) %>) %</div>
		  <div class="progress-bar bg-warning" role="progressbar" style="width: <%= ((bills/size)*100.0) %>%">Bills (<%= (int)((bills/size)*100.0) %>) %</div>
		  <div class="progress-bar bg-secondary" role="progressbar" style="width: <%= ((others/size)*100.0) %>%">Other (<%= (int)((others/size)*100.0) %>) %</div>
		</div>
	</div>
	<% } %>
	<div class="m-2 text-right">
		<button type="button" class="btn btn-danger btn2" onclick="showAddExpense()">Add Expense</button>
	</div>
...
<h4>Add Expense </h4>
      ...
      <label for="expDate" class="row-sm-2 col-form-label">Expense Date:</label>
      
        <input type="date" id="expDate"class="form-control" name="expDate" placeholder="Enter Expense Date" required>
        </div>
        <div class="form-group">
        <label for="expDesc" class="row-sm-2 col-form-label">Expense Description:</label>
        <textarea name="expDesc" id="expDesc" class="form-control" rows="5" placeholder="Enter Expense Description" required></textarea>
      </div>
					<div class="row ">
						<div class="form-group col-md-6">
							<label for="expType">Expense Type:</label> 
							<select id="expType" class="form-control" name="expType">
							...
							<div class="form-group col-md-6">
								...
								<input type="number" min="1" class="form-control" id="total"
									name="total" placeholder="Total Expense" required>
							</div>
	...
	<div class=" align-center float-right">
        ...
        <button type="submit" class="btn btn-danger" onclick="makeAddExpenseDisappear()">Submit</button>
      </div>
    ...
    <!-- Modal -->
<div class="modal fade" id="delete_list" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
